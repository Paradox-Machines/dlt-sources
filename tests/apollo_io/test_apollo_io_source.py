"""Integration tests for the apollo_io_source factory.
...
``apollo_io_source`` (and any declared paginator classes) and the ``_fixture``
...
@responses.activate
def test_apollo_io_source_runs_against_duckdb(tmp_pipeline) -> None:
    _install_fixture_routes()

    info = tmp_pipeline.run(apollo_io_source(api_key="apollo_io_dummy", base_url=_BASE))
...
def test_returns_expected_resources() -> None:
    src = apollo_io_source(api_key="apollo_io_dummy")
    assert {r.name for r in src.resources.values()} == _EXPECTED_RESOURCES


def test_resource_dispositions() -> None:
    src = apollo_io_source(api_key="apollo_io_dummy")
    assert src.resources["contacts"].write_disposition == "append"
    assert src.resources["accounts"].write_disposition == "replace"
    assert src.resources["people"].write_disposition == "replace"
    assert src.resources["opportunities"].write_disposition == "replace"
    assert src.resources["sequences"].write_disposition == "replace"
    assert src.resources["users"].write_disposition == "replace"
    assert src.resources["email_accounts"].write_disposition == "replace"
    assert src.resources["labels"].write_disposition == "replace"
